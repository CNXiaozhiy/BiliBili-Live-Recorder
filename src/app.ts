...
function initSqlite() {
  return new Promise<void>((resolve) => {
    db.serialize(() => {
      ...
      db.run(`INSERT OR IGNORE INTO settings (name, value) VALUES (?, ?)`, [
        "bilibili_cookie",
        store.bilibili_cookie,
      ]);
      db.run(`INSERT OR IGNORE INTO settings (name, value) VALUES (?, ?)`, [
        "bilibili_refresh_token",
        store.bilibili_refresh_token,
      ]);
      ...
      config.bot.admin.forEach((admin) => {
        db.run(`INSERT OR IGNORE INTO bot_admin (user_id, permission) VALUES (?, ?)`, [
          admin.qid,
          admin.permission,
        ]);
        db.run(`UPDATE bot_admin SET permission = ? WHERE user_id = ?`, [
          admin.permission,
          admin.qid,
        ]);
      });
...
function initStoreData() {
  return new Promise<void>(async (resolve) => {
    ...
    for (const row of rows) {
      row.value = (
        await tools.all<[{ value: string }]>("SELECT value FROM settings WHERE name = ?", [
          row.name,
        ])
      )[0].value;
...
const tools = {
  ...
  async isAdmin(user_id: number): Promise<{ isAdmin: boolean; permission: number }> {
    return new Promise((resolve, reject) => {
      this.all<DBBotAdminTableRows>("SELECT * FROM bot_admin WHERE user_id = ?", [user_id]).then(
        (rows) => {
          if (rows.length > 0) {
  ...
  async getQSRoomIDByGID(group_id: number): Promise<number> {
    return new Promise((resolve, reject) => {
      tools
        .all<DBQuickSubscribeTableRows>("SELECT * FROM quick_subscribe WHERE group_id = ?", [
          group_id.toString(),
        ])
        .then((rows) => {
          if (rows.length > 0) {
...
const app = async () => {
  ...
  if (qbot) {
    ...
    // qbot.action('switch', { qid: 3826082036 });
    ...
    if (data.post_type === "message") {
      ...
      if (data.message_type === "group") {
        ...
        function subscribe(room_id: number, group_id: number, user_id: number) {
          tools
            .all("SELECT * FROM subscribe WHERE room_id = ? AND group_id = ? AND user_id = ?", [
              room_id,
              group_id,
              user_id,
            ])
            .then((rows) => {
              if (rows.length > 0) {
                ...
                db.run("INSERT INTO subscribe (room_id, group_id, user_id) VALUES (?, ?, ?)", [
                  room_id,
                  group_id,
                  user_id,
                ]);
        ...
        function unsubscribe(room_id: number, group_id: number, user_id: number) {
          tools
            ...
            .then((rows) => {
              if (rows.length > 0) {
                db.run("DELETE FROM subscribe WHERE room_id = ? AND group_id = ? AND user_id = ?", [
                  room_id,
                  group_id,
                  user_id,
                ]);
        ...
        if (data.message[0].type === "text") {
          ...
          if (level2Event) {
            if (command === "q") {
              ...
              try {
                ...
                tools
                  .all("SELECT * FROM subscribe WHERE group_id = ? AND user_id = ?", [gid, qid])
                  .then((rows) => {
                ...
                error.message !== $t("TEXT_CODE_7f34883a") &&
                  alertError(error, $t("TEXT_CODE_0aef44bd"));

                qbot?.action("send_group_msg", {
                  group_id: gid,
                  message: [{ type: "text", data: { text: $t("TEXT_CODE_8bbbb4a0") } }],
                });
              }
              ...
              try {
                ...
                if (data.message[0].data.text === $t("TEXT_CODE_command.yes")) {
                  tools.getQSRoomIDByGID(gid).then((room_id) => {
                    if (room_id !== 0) {
                      ...
                      qbot?.action("send_group_msg", {
                        group_id: gid,
                        message: [{ type: "text", data: { text: $t("TEXT_CODE_61a034b7") } }],
                      });
                    }
                  });
                  return;
                ...
                error.message !== $t("TEXT_CODE_78e760ba") &&
                  alertError(error, $t("TEXT_CODE_da1deb89"));
                ...
                qbot?.action("send_group_msg", {
                  ...
                  message: [
                    {
                      type: "text",
                      data: { text: $t("TEXT_CODE_cf398d8b", { replace: { err: error.message } }) },
                    },
                  ],
                });
              }
              ...
              tools
                ...
                .then((rows) => {
                  try {
                    ...
                    error.message !== $t("TEXT_CODE_7f34883a") &&
                      alertError(error, $t("TEXT_CODE_3291c3b3"));

                    qbot?.action("send_group_msg", {
                      group_id: gid,
                      message: [{ type: "text", data: { text: $t("TEXT_CODE_7f34883a") } }],
                    });
                  }
                });
              ...
              tools
                ...
                .then((rows) => {
                  try {
                    ...
                    error.message !== $t("TEXT_CODE_7f34883a") &&
                      alertError(error, $t("TEXT_CODE_9307956f"));
              ...
              uploader.createTask(json.uploader.params).then((task) => {
                qbot?.action("send_group_msg", {
                  ...
                  message: [
                    ...
                    {
                      type: "text",
                      data: { text: $t("TEXT_CODE_8e44f1b0", { replace: { id: task.id } }) },
                    },
                  ],
                });
                ...
                message: [
                  {
                    ...
                    data: {
                      text:
                        ...
                        "统计信息 已清理\n" +
                        "损坏文件: " +
                        result.statisticalInformation.damagedFiles +
                        "\n" +
                        "异常遗留文件: " +
                        result.statisticalInformation.exceptionallyLeftFiles +
                        "\n" +
                        "总清理文件数: " +
                        (result.statisticalInformation.damagedFiles +
                          result.statisticalInformation.exceptionallyLeftFiles) +
                        "\n\n" +
                        "未恢复的文件数量: " +
                        result.statisticalInformation.unrecoveredFiles +
            ...
            if ("id" in result) {
              tools.isAdmin(qid).then(async (res) => {
                if (res.isAdmin) {
                  ...
                  tools
                    .all("SELECT * FROM quick_subscribe WHERE group_id = ?", [gid])
                    .then((rows) => {
                      if (rows.length <= 0) {
                        db.run("INSERT INTO quick_subscribe (group_id, room_id) VALUES (?, ?)", [
                          gid,
                          room_id,
                        ]);
                      } else {
                        db.run("UPDATE quick_subscribe SET room_id = ? WHERE group_id = ?", [
                          room_id,
                          gid,
                        ]);
                      }
          ...
          matchCommand(
            command,
            [$t("TEXT_CODE_command.add_admin"), $t("TEXT_CODE_command.add_admin_2")],
            (result) => {
              if ("id" in result) {
                ...
                tools.isAdmin(qid).then((res) => {
                  if (res.isAdmin && res.permission >= 10) {
                    tools
                      .all("SELECT * FROM bot_admin WHERE user_id = ?", [user_id])
                      .then((rows) => {
                        if (rows.length <= 0) {
                          const permission =
                            "permission" in result ? parseInt(result.permission) : 1;
                          ...
                          db.run("INSERT INTO bot_admin (user_id, permission) VALUES (?, ?)", [
                            user_id,
                            permission,
                          ]);
                    ...
                    qbot?.action("send_group_msg", {
                      ...
                      message: [
                        { type: "reply", data: { id: message_id.toString() } },
                        {
                          type: "text",
                          data: {
                            text: res.isAdmin ? $t("TEXT_CODE_e01af5e0") : $t("TEXT_CODE_84eda865"),
                          },
                        },
                      ],
                    });
                    ...
                    qbot?.action("send_group_msg", {
                      ...
                      message: [
                        { type: "reply", data: { id: message_id.toString() } },
                        {
                          ...
                          data: {
                            text:
                              user_admin.permission >= res.permission
                                ? $t("TEXT_CODE_d22f6d6a")
                                : $t("TEXT_CODE_662f897e"),
                          },
                        },
                      ],
                    });
          ...
          matchCommand(command, [$t("TEXT_CODE_command.subscribe")], () => {
            tools.getQSRoomIDByGID(gid).then(async (room_id) => {
              if (room_id) {
                ...
                const text = room_id
                  ? $t("TEXT_CODE_09460d60", {
                      replace: { name: up_info.card.name || "未知", id: room_id },
                    })
                  : $t("TEXT_CODE_61a034b7");
                qbot?.action("send_group_msg", {
                  group_id: gid,
          ...
          matchCommand(
            command,
            [$t("TEXT_CODE_command.rec_stop_1"), $t("TEXT_CODE_command.rec_stop_2")],
            async () => {
              if (!(await tools.isAdmin(qid)).isAdmin) {
                qbot?.action("send_group_msg", {
                  group_id: gid,
              ...
              tools
                ...
                .then((rows) => {
                  if (rows.length > 0) {
                    ...
                    message.push({
                      type: "text",
                      data: { text: "q. " + $t("TEXT_CODE_8ed4f929") },
                    });
    ...
    tools.all("SELECT * FROM subscribe").then((rows) => {
      ...
      map_ARecorders.forEach((aRecorder: BiliLiveAutoRecorder, room_id: number) => {
        if (!rows.find((row) => row.room_id === room_id)) {
          logger.info(
            $t("TEXT_CODE_19d6f339", { replace: { code: colorize("yellow", room_id.toString()) } })
          );
          aRecorder.destroy();
      ...
      map_room.forEach((room_subs: { group_id: number; user_id: number }[], room_id: number) => {
        ...
        if (aRecorder) {
          ...
          logger.info(
            $t("TEXT_CODE_0fe45b42", { replace: { code: colorize("yellow", room_id.toString()) } })
          );
        ...
        aRecorder.monitor.on("status-change", (info) => {
          logger.info(
            $t("TEXT_CODE_5b289dd8", {
              replace: {
                roomId: colorize("blue", room_id.toString()),
                status: statusToString("liveStatus", info.live_status),
              },
            })
          );
        });
            ...
            const message = [
              {
                type: "text",
                data: {
                  text: $t("TEXT_CODE_fcd1f98f", {
                    replace: {
                      id: room_id,
                      status: statusToString(
                        "transcodeMP4Status",
                        aRecorder.recorder.transcodeMP4 ? 1 : 0
                      ),
                    },
                  }),
                },
        ...
        aRecorder.recorder.on("rec-transcode-end", (files) => {
          ...
          map_groupSend.forEach(async (user_ids, group_id) => {
            const message = [
              {
                type: "text",
                data: { text: $t("TEXT_CODE_9753a930", { replace: { id: room_id } }) },
              },
            ];
            qbot?.action("send_group_msg", { group_id, message });
          });
        ...
        aRecorder.recorder.on("rec-all-end", async (file) => {
          ...
          const format_time_start = moment(aRecorder.recorder.recStartTime).format(
            "yyyy-MM-DD HH:mm:ss"
          );
          const format_time_end = moment(aRecorder.recorder.recEndTime).format(
            "yyyy-MM-DD HH:mm:ss"
          );
          ...
          const params = {
            ...
            video: {
              ...
              description: $t("TEXT_CODE_560f94fc", {
                replace: {
                  id: room_info.room_id,
                  description: room_info.description || "无",
                  title: room_info.title,
          ...
          uploader.createTask(params).then(({ id, upload }) => {
              ...
              const message = [
                {
                  type: "text",
                  data: {
                    text: $t("TEXT_CODE_7ff48a7e", { replace: { id: room_id, taskID: id } }),
                  },
                },
              ];
              qbot?.action("send_group_msg", { group_id, message });
            ...
            upload()
              ...
              .catch((err) => {
                ...
                fs.writeFileSync(
                  path.join(recover_folder, recover_file),
                  JSON.stringify(
                    {
...
//   if (command === "exit") {
//     rl.close();
//     logger.warn('[EXIT] 开始清理当前所有正在录制的直播间');

//     for (const [room_id, aRecorder] of map_ARecorders) {
//         aRecorder.recorder.stop().force();
//         aRecorder.monitor.stopMonitor();

//         aRecorder.recorder.off('all')
//         await new Promise<void>((resolve) => {
//             aRecorder.recorder.on('rec-all-end', resolve)
//         })
//     }
//     logger.warn('[EXIT] 清理完成');
//   }
// };
